...
SYSTEM_GENERATE_VERSION = "jos-sdk-python-20161129"
...
class RestApi(object):
    ...
    def __init__(self, domain='103.235.242.215', port = 8080):
        #=======================================================================
        # 初始化基类
        ...
        if(jos.getDefaultAppInfo()):
            self.__app_key = jos.getDefaultAppInfo().appkey
            self.__secret = jos.getDefaultAppInfo().secret
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        parameters = dict(sys_parameters, **{"360buy_param_json":json.dumps(application_parameter)})
        parameters[P_SIGN] = sign(self.__secret, parameters)
        header = self.get_request_header();
        parameters = urllib.parse.urlencode(parameters)
        connection.request(self.__httpmethod, "", parameters, headers=header)
